...
#include "sys/log.h"
#define LOG_MODULE "TCPS"
#define LOG_LEVEL LOG_LEVEL_INFO

#define WITH_SERVER_REPLY  1
#define TCP_SERVER_PORT 1010
...
PT_THREAD(handle_connection(struct psock *p))
{
  ...
  /* Check if it special request */
  volatile uint16_t datalen = PSOCK_DATALEN(p);
  if(datalen >= 2 && buffer[0] == 'E') {
    uint8_t buf[10];
    ...
    LOG_INFO("Sending data %d bytes to source\n", datalen);
  } else {
...
PROCESS_THREAD(tcp_server_process, ev, data)
{
   ...
   */
  tcp_listen(UIP_HTONS(TCP_SERVER_PORT));

  LOG_INFO("Register TCP server, port %d\r\n", TCP_SERVER_PORT);
