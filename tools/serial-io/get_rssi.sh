...
if [ -z "$1" ] || [ -z "$2" ]; then
  echo "Usage : ./get_rssi.sh nodeaddr nodeport"
  # ./get_rssi.sh fd00::f6ce:3606:8f9f:f3d2 7777
  exit 1
fi
...
if [ "$NNODES" -lt "$NNODESMAX" ]; then
	...
	while [ "$COUNTER" -le "$(("NNODES"))" ]; do       
		# host address
		HOSTADDRESS=$PREFIX$(printf '%02x::1' $COUNTER)
		# get ping time
		PING=$(ping6 -c1 -W1 "$HOSTADDRESS" | grep -oP '(?<=time\=).*')
		if [ "$?" -ne 0 ] || [ -z "$PING" ] ; then
    			PING="Offline"
		fi
		# print
		echo "$COUNTER" host address : "$HOSTADDRESS"  ping : "$PING"
		sleep 0.1
        	COUNTER="$(("COUNTER"+1))"	 
	done
fi
