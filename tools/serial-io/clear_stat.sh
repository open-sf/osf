...
# Clear statistic counters on all Nodes
if [ "$NNODES" -lt "$NNODESMAX" ]; then
	...
	while [ "$COUNTER" -le "$(("NNODES"))" ]; do       
		...
		if [ "$?" -ne 0 ] || [ -z "$PING" ] ; then
    			...
    			# ping OK, clear counters
			CLEARSTAT=$(echo -n "?SZ" | nc -6u -w1 -W1 "$NODEADDRESS" "$THENODEPORT")
   			if [ "$?" -ne 0 ] || [ -z "$CLEARSTAT" ] || [ "${CLEARSTAT:0:3}" != "!SZ" ]; then
    			CLEARSTAT="Offline"
			else
			CLEARSTAT="OK"
			fi	
		...
		# print
		echo "$COUNTER" node address : "$NODEADDRESS"  ping : "$PING" clear statistics : "$CLEARSTAT"
		sleep 0.1
        	COUNTER="$(("COUNTER"+1))"	 
	done
fi	
