...
# declare -a phy_arr=('BLE_2M' 'BLE_1M')
declare -a jam_arr=(0 1 3)
declare -a pwr_arr=(ZerodBm, Neg8dBm, Neg20dBm)
declare -a data_arr=(8 64 118 248)
...
for pwr in "${pwr_arr[@]}"; do
  for d in "${data_arr[@]}"; do
    for j in "${jam_arr[@]}"; do
      for p in "${phy_arr[@]}"; do
        ./dcube.sh -POST KEY=$KEY TARGET=nrf52840 BOARD=dk -e osf -n "OSF" -p $DIS_PROTO --dur=$DUR --j=${j} --s=0 --layout=$LAYOUT --data=${d} -d "BV ${d}B ${p}" LENGTH=${d} PHY=PHY_"${p}" PWR=$PWR PERIOD=$PERIOD CHN=1 LOGGING=$LOGGING GPIO=$GPIO NTX=$NTX NSLOTS=$NSLOTS RNTX=0 PROTO=OSF_PROTO_BCAST DCUBE=1 -sha
      done
    done
  done
done
