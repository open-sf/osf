...
if [[ -v IPV6 ]]; then
  ...
  sudo ../../../tools/serial-io/tunslip6 fd0$device_id::1/64 -s $device_port -B 921600 -d1 &
  sleep 5
  ...
  len=`expr ${#id_arr[@]} - 1`
  sudo ip route add fd00::/64 via fd0$device_id::$device_mac dev $tundev
  echo "sudo ip route add fd00::/64 via fd0$device_id::$device_mac dev $tundev"
  for i in $(seq 0 $len); do
    if [ ${mac_arr[$i]} != $device_mac ]; then
      sudo ip route add fd0${id_arr[$i]}::/64 via fd0$device_id::$device_mac dev $tundev
      echo "sudo ip route add fd0${id_arr[$i]}::/64 via fd0$device_id::$device_mac dev $tundev"
    fi
  done
fi
